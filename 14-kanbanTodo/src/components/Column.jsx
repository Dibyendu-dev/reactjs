...
export default function Column({ column }) {
  ...
  const tasks = useSelector(state => column.taskIds.map(id => state.board.tasks[id]));
  const [newTask, setNewTask] = useState('');

  return (
    <div className="w-64 bg-gray-200 dark:bg-gray-900 rounded p-4 m-2">
      <div className="flex justify-between mb-2">
        <h2 className="text-lg font-bold text-black dark:text-white">{column.title}</h2>
      ...
      <form
        ...
        className="mt-2"
      >
        <input value={newTask} onChange={e => setNewTask(e.target.value)} placeholder="New task..." className="w-full p-2 rounded" />
      </form>
    </div>
  );
}